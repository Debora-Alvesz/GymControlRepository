...
public class PlanoService implements PlanoIService{
    ...
    public Plano save(Plano plano) {
       //Verifica se já existe esse ID no banco
        ...
        logger.info("[SERVICE] PlanoService - Plano cadastrado com sucesso.");
    ...
    public List<Plano> findAll() {
        ...
        logger.info("[SERVICE] PlanoService - Buscando lista de planos");
    ...
    public void delete(Long id) {
        ...
        logger.info("[SERVICE] PlanoService - Solicitação de exclusão do plano. ID: {}", id);
        if (!planoRepository.existsById(id)) {
            // Se não existir, lança um erro e avisa o Controller
            logger.error("[SERVICE] PlanoService - Tentativa de exclusão de plano inexistente. ID: {}", id);
        ...
        logger.info("[SERVICE] PlanoService - Plano excluído com sucesso. ID: {}", id);
    }
    @Override
    public Plano update(Long id, Plano plano) {
        //Verifica se o plano que queremos editar realmente existe
        logger.info("[SERVICE] PlanoService - Iniciando atualização do plano. ID: {}", id);
        if (!planoRepository.existsById(id)) {
            logger.error("[SERVICE] PlanoService - Plano não encontrado para atualização. ID: {}", id);
